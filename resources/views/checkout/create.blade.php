<x-app-layout>
    ...
    <div class="flex flex-col md:flex-row items-start justify-start bg-white w-full border border-zinc-500 rounded-2xl">
        ...
        <div class="w-full h-full md:w-2/5 flex flex-col gap-5 p-5 md:border-r border-zinc-400">
            ...
            <p class="text-rose-600 text-lg font-semibold">Rp{{ number_format($product->harga, 0, ',', '.') }},00</p>
        ...
        <div class="w-full md:w-3/5">
            <form action="{{ route('order.store', $product->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <label class="block">
                    ...
                    <input type="text" name='nama_pemesan' required placeholder="masukkan nama penerima belanjaan"
                        class="mt-1 block w-full rounded-lg bg-transparent border border-zinc-700 px-3 py-2 placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-zinc-600" />
                </label>
                ...
                <div class="flex gap-4 items-center w-full">
                    <label class="flex-1">
                        ...
                        <input type="text" value="Rp{{ number_format($product->harga, 0, ',', '.') }},00"
                            name="total_harga" required readonly
                            class="mt-1 w-full rounded-lg bg-transparent text-rose-600 border border-zinc-700 px-3 py-2 placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-zinc-600" />
                    </label>

                    <label class="flex-1">
                        <span class="text-sm">Jumlah produk yang dibeli</span>
                        <input type="number" min="1" max="{{ $product->stok }}" value="1" name="jumlah"
                            required
                ...
                <label for="kurir" class="block">
                    ...
                    <select name="kurir"
                        class="mt-1 block w-full rounded-lg bg-white border border-zinc-700 px-3 py-2 placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-zinc-600"
                        required>
